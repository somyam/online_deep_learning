...
class WeatherForecast:
    ...
    def find_min_and_max_per_day(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        min_per_day = self.data.min(dim=1)[0]
        max_per_day = self.data.max(dim=1)[0]
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        daily_changes = daily_averages[1:] - daily_averages[:-1]
        return daily_changes.min()
  ...
  max_diff_indices]
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        """
        return torch.argmin(torch.abs(self.data-t).sum(dim=1))
